...
State a mod can override (colours, behaviour hooks) belongs on the interface *that mod itself
implements* (e.g. `PonderPlugin`), scoped to that mod's own scenes — never a global static setter.
Global mutable state shared by the whole JVM means one mod's choice silently repaints every other
mod's output; that's not a convenience worth the coupling.
