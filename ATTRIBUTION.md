...
| `foundation.ui.PonderUI.renderNextUp`/`tickNextUp` | `renderNextUp` — 50-tick warmup and the box above the right control rising 5px as it eases in. Deviation: PonderLib uses its own restrained attention rim instead of upstream's arrow flash. Not ported: `isNextUpEnabled()` (no API to set it here) |
| `foundation.element.InputWindowElement`, `api.scene.InputElementBuilder`, `render.PonderSpeechBox` | `InputWindowElement`/`InputElementBuilder` and Catnip's speech box — anchored input hints with a directional tail. Icons are NOT ported (Create's `ICON_LMB`/`ICON_RMB`/`ICON_SCROLL` are All-Rights-Reserved assets); `showing(ScreenElement)` is absent for the same reason. `drop()` is PonderLib's own code-drawn/text extension; upstream has no dedicated drop-key control |
...
| `foundation.ui.PonderUI.renderTimeline`/`renderKeyframeMarks`/`drawKeyframeMark`/`hoveredKeyframeIndex` | `PonderProgressBar` — 220px centred placement, `BoxElement` frame, `(-2,-2)` draw frame, `BAR_COLORS` two-band fill, `chase(target, .5f, EXP)` easing, marks hanging below the bar, hitbox reaching 20px below, and its own `width + 2` vs `width + 4` denominator mismatch. Deviation: marker growth, playhead proximity and the pixel direction chevron are PonderLib's own motion design |
...
| `foundation.ui.PonderButton` | `PonderButton` (over Catnip's `BoxWidget`) — padded 28×28 hit area around the nominal 20×20 box, channel-quantized `.6 EXP` hover transition, `.1 EXP` flash/fade chasers, item/tag transforms and z ordering. Icons are NOT ported: upstream's come from Create's `widgets` texture atlas, whose `assets/` are All Rights Reserved. The lower control row's centre-out entrance, lift, squash, rim and directional icon parallax are PonderLib's own |
| `foundation.ui.PonderUI` tag sidebar, `PonderTagScreen`, `PonderStreak` | Ponder's scene tag buttons and tag-detail screen plus Catnip `LayoutHelper` and `UIRenderHelper.streak` — anchors, row distribution, frames, clipping, transforms, text wrapping and tag hover timing. Colours intentionally remain PonderLib's per-plugin `PonderColorScheme`; Create assets are not used. Back navigation deliberately replaces Catnip's breadcrumb ribbon with PonderLib's own code-drawn echo chevrons |
...
- `PonderTagIndexScreen`/`PonderIndexScreen`'s tag filter — index→tag→scene navigation graph and
  namespace grouping only. This refers to the older all-tags index screens; the separate
  scene-sidebar/tag-detail pair is now the close port listed above. Index pagination and Catnip's
  framebuffer screen transition are still not ported.
